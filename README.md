...
3. When collinearity issues arise during the calculation of conditional or joint effects (for example, when using `--cojo-cond` or `--cojo-joint`), the original GCTA program simply terminates without producing output. Our program, in contrast, iteratively removes the most problematic SNPs until the collinearity issue is resolved. A `.warning` suffix is added to the output filename to indicate this behavior. The same logic also applies when users run iterative selection starting from a fixed set of SNPs (`--fix` or `--fix-snp`).
...
| ------------------ | -------------------------------------------------------------- | ---------------- |
| `--cojo-file`      | GWAS summary statistics file for each cohort<br>Must be in GCTA-COJO format, with a header of `SNP A1 A2 freq b se p N`    | `Required`       |
