# Manc-COJO <a href="https://smartbiomed.dk/" target="_blank" rel="noopener noreferrer"><img align="right" alt="SMARTbiomed software review badge" src="https://raw.githubusercontent.com/SMARTbiomed/software-review/refs/heads/master/docs/data/smartbiomed-software.svg" /></a>
...
3. By default, our software does not generate `.cma.cojo` and `.ldr.cojo` files, as these outputs can be very large and are not required for most use cases. Use `--output-all` to enable all output files, which will also record unqualified SNPs in the corresponding `.badsnps` files.
