...
{   
    ...
    if (params.if_cond_mode) {
        vector<string> temp;
        skim_SNP(params.cond_file, 1, false, temp);

        initialize_candidate_SNP(temp);
    ...
    if (!params.fix_file.empty() || !params.fix_SNPs.empty()) {
        vector<string> temp;
        get_user_SNP(params.fix_file, params.fix_SNPs, temp);
    ...
    vector<pair<string, int>>().swap(shared.goodSNP_table); // free memory
