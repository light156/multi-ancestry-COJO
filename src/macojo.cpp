...
{   
    ...
    if (params.if_cond_mode) {
        vector<string> given_list;
        skim_SNP(params.cond_file, 0, false, given_list);

        initialize_candidate_SNP(given_list);
    ...
    if (!params.fix_file.empty() || !params.fix_SNPs.empty()) {
        vector<string> given_list = params.fix_SNPs;
        skim_SNP(params.fix_file, 0, false, given_list);
