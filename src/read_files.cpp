...
{   
    if (!first_column && !second_column)
        LOGGER.e(0, "skim_file: at least one of first_column or second_column must be true");
    ...
    if (adjacent_find(str_list.begin(), str_list.end()) != str_list.end())
        LOGGER.e(0, "Duplicate names in [" + filename + "], please check", *adjacent_find(str_list.begin(), str_list.end()));
}
...
{  
    ...
    if (!Meta) LOGGER.e(0, "cannot open sumstat file [" + cojo_file + "] to read");
    LOGGER << "Reading GWAS summary-level statistics from [" + cojo_file + "] ..." << endl;
    ...
    if (split_string(str_buf, vs_buf) != 8) LOGGER.e(0, "format error in sumstat file");
    ...
    while (Meta >> SNP_buf >> A1_buf >> A2_buf >> freq_buf >> b_buf >> se_buf >> p_buf >> N_buf) {
        ...
        if ((b_buf == "NA" || b_buf == "." || se_buf == "NA" || se_buf == "." || se_buf == "0" || \
            ...
            if (iter != shared.goodSNP_index_map.end()) {
                LOGGER.w(1, "removed, invalid value in sumstat file", SNP_buf);
...
{   
    ...
    if (!params.keep_file_list.empty()) {
        ...
        all_ids = temp_ids;
        LOGGER.i(0, "individuals after keeping individuals", to_string(all_ids.size()));
    }
    ...
    if (!params.remove_file_list.empty()) {
        ...
        all_ids = temp_ids;
        LOGGER.i(0, "individuals after removing individuals", to_string(all_ids.size()));
    }

    if (all_ids.size() == 0)
        LOGGER.e(0, "no individuals remaining after applying keep/remove individual files");

    // nothing changed
    if (fam_indi_num == all_ids.size()) {
        valid_indi_num = fam_indi_num;
        genotype.initialize_mask(fam_indi_num);
        LOGGER.i(0, "individuals will be used for analysis", to_string(fam_indi_num));
        return;
    }
    ...
    if (valid_indi_num > fam_indi_num / 2) {
        LOGGER.i(0, "Read all individuals into memory and mask unwanted individuals");
        genotype.initialize_mask(fam_indi_num);
        ...
        LOGGER.i(0, "Only read individuals to be used into memory");
    ...
    Fam.close();
    LOGGER.i(0, "individuals will be used for analysis", to_string(valid_indi_num));
}
...
{   
    ...
    while (Frq >> str_buf >> SNP_buf >> A1_buf >> A2_buf >> freq_buf >> str_buf) {
        ...
        if (shared.A1_ref[ref_index] == A1_buf && shared.A2_ref[ref_index] == A2_buf) {}
            ...
            LOGGER.w(1, "removed, different A1 and A2 from ref BIM file, please check", SNP_buf);
            shared.goodSNP_index_map.erase(iter);
            continue;
        } 
    ...
    while (Bim >> chr_buf >> SNP_buf >> str_buf >> ibuf >> A1_buf >> A2_buf) {
        ...
        if (is_ref_cohort) {
            ...
            if (shared.A1_ref[ref_index] == A1_buf && shared.A2_ref[ref_index] == A2_buf) {}
                ...
                LOGGER.w(1, "removed, different A1 and A2 between two BIM files, please check", SNP_buf);
                shared.goodSNP_index_map.erase(iter);
                continue;
            }

            if (chr_buf != shared.chr_ref[ref_index]) {
                LOGGER.w(1, "removed, different chromosome between two BIM files, please check", SNP_buf);
                shared.goodSNP_index_map.erase(iter);
                continue;
            }

            if (ibuf != shared.SNP_pos_ref[ref_index]) {
                LOGGER.w(1, "removed, different SNP position between two BIM files, please check", SNP_buf);
                shared.goodSNP_index_map.erase(iter);
                continue;
            }      
        }
...
{   
    ...
    if (!params.extract_file.empty()) {
        ...
        if (commonSNP_all_cohorts.size() == 0)
            LOGGER.e(0, "Input extract SNP file has no SNPs", params.extract_file);
    ...
    for (int n = 0; n < cohorts.size(); n++) {
        ...
        LOGGER.i(0, "SNPs in [" + PLINK_file + ".bim]", to_string(SNP_PLINK.size()));
        ...
        LOGGER.i(0, "SNPs in [" + sumstat_file + "]", to_string(SNP_sumstat.size()));
    ...
    if (!params.exclude_file.empty()) {
        ...
        commonSNP_all_cohorts.swap(temp);
        LOGGER.i(0, "common SNPs after excluding user-specified SNPs\n", to_string(commonSNP_all_cohorts.size()));
    }
    ...
    for (auto iter = shared.goodSNP_index_map.begin(); iter != shared.goodSNP_index_map.end(); ) {
        ...
        if (erase_flag) {
            // LOGGER.w(1, "removed, rare SNP based on frequency threshold", iter->first);
