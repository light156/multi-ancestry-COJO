...
{   
    ...
    for (int n = 0; n < cohorts.size(); n++) {
        ...
        skim_SNP(params.cojo_file_list[n], 0, true, SNP_sumstat);
    ...
    if (!params.extract_file.empty() || !params.extract_SNPs.empty()) {
        vector<string> extract_SNP = params.extract_SNPs;
        skim_SNP(params.extract_file, 0, false, extract_SNP);
        LOGGER.i("user-specified SNPs to include", extract_SNP.size());

        vector<string> temp;
        set_intersection(common_SNP.begin(), common_SNP.end(), extract_SNP.begin(), extract_SNP.end(), back_inserter(temp));
        common_SNP.swap(temp);
    ...
    if (!params.exclude_file.empty() || !params.exclude_SNPs.empty()) {
        vector<string> exclude_SNP = params.exclude_SNPs;
        skim_SNP(params.exclude_file, 0, false, exclude_SNP);
        LOGGER.i("user-specified SNPs to exclude", exclude_SNP.size());
